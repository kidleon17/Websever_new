...
void Log :: write_log(int level, const char *format, ...) {
    ...
    if(m_today != my_tm.tm_mday || m_count % m_split_lines == 0) {//按天划分
        char new_log[256];
        fflush(m_fp);
        fclose(m_fp);
        char tail[16] = {0};

        snprintf(tail, 16, "%d_%02d_%02d_", my_tm.tm_year + 1900, my_tm.tm_mon + 1, my_tm.tm_wday);
